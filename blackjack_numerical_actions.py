...
ACTION_LOOKUP = ["hit", "stand", "double", "split", "surrender"]
...
class BlackjackEnv: 
    ...
    def get_state(self):
        ...
        # Represent state as a NumPy array: [player_sum, dealer_card, usable_ace]
        state = np.array([
            player_sum / 21,              # Normalize to [0, 1]
            ((dealer_card - 2) % 10)  / 9,             # Normalize to [0, 1]
    ...
    def get_legal_actions(self):
        ...
        actions = [["hit", "stand", "double", "split", "surrender"].index(action) for action in actions]
        return actions
    ...
    def step(self, action_idx):
        ...
        assert action_idx in legal, f"Illegal action: {action_idx}, legal: {legal}"

        action_str = ACTION_LOOKUP[action_idx]
